...
public:
  database_manager() : log_(redlog::get_logger("db")) { log_.info("Database manager initialized"); }

  void connect(const std::string& host, int port) {
    auto conn_log = log_.with_field("host", host).with_field("port", port);
  ...
  void execute_query(const std::string& sql) {
    ...
    auto query_log = log_.with_name("query").with_field("query_id", ++query_id);
    ...
    query_log.debug_f("Query completed in %d ms, affected %d rows", static_cast<int>(duration_ms), 42);
    query_log.trace("Query performance", redlog::field("duration_ms", duration_ms), redlog::field("rows_affected", 42));
  }
};
...
void handle_http_request(const http_request& req) {
  ...
  try {
    ...
    if (req.path == "/api/users") {
      ...
      request_log.debug("Response details", redlog::field("status_code", 200), redlog::field("response_size", 1024));
    ...
    request_log.debug("Error details", redlog::field("error", e.what()), redlog::field("status_code", 500));
  }
}

// Thread safety demonstration
void worker_thread(int thread_id) {
  auto log = redlog::get_logger("worker").with_field("thread_id", thread_id);
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    case 1:
      log.verbose(
          "Processing item with verbose details", redlog::field("item", i), redlog::field("memory_mb", 128 + i * 10)
      );
      break;
    case 2:
      log.debug(
          "Processing item with debug info", redlog::field("item", i), redlog::field("cpu_percent", 15.5 + i * 2.1)
      );
      break;
    case 3:
      log.trace(
          "Processing item with trace details", redlog::field("item", i),
          redlog::field(
              "timestamp",
...
int main() {
  ...
  log.info(
      "Performance comparison completed", redlog::field("iterations", iterations),
      redlog::field(
          "restrictive_time_us", std::chrono::duration_cast<std::chrono::microseconds>(restrictive_time).count()
      ),
